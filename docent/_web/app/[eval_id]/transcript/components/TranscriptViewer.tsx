...
// Add DiffDisplay component to show diff results below citations
const DiffDisplay: React.FC<{
  ...
  // Get all diffs that involve this agentRunId
  const relevantDiffs = useMemo(() => {
    ...
    for (const [pairKey, diffData] of Object.entries(diffMap)) {
      const [id1, id2] = pairKey.split('___');
      
      // Check if this agentRunId is part of this diff pair
      if (id1 === agentRunId) {
        diffs.push({
          pairKey,
          transcriptIdx: 0,
          claim: diffData.claim,
          evidence: diffData.evidence,
        });
      } else if (id2 === agentRunId) {
        diffs.push({
          pairKey,
          transcriptIdx: 1,
          claim: diffData.claim,
    ...
    // Filter diffs that have evidence citations referencing this block
    return diffs.filter(diff => 
      diff.evidence.some(evidenceItem => 
        evidenceItem.citations?.some(citation => citation.block_idx === blockIndex && citation.transcript_idx === diff.transcriptIdx)
      )
    );
  }, [diffMap, agentRunId, blockIndex]);
  ...
  return (
    <div className="ml-6 mt-2 mb-3 pt-1.5 border-t border-purple-100 text-xs">
      <div className="flex items-center mb-1">
      ...
      <div>
        {relevantDiffs.map((diff, diffIdx) => {
          // Create pairs of claims and evidence
          const diffPairs = diff.claim.map((claim, idx) => ({
            claim,
            evidence: diff.evidence[idx] || null,
          }));

          return (
            <div key={diff.pairKey} className="mb-2">
              
              {diffPairs.map((pair, pairIdx) => {
                // Only show pairs where evidence has citations for this block
                const hasRelevantCitation = pair.evidence?.citations?.some(
                  citation => citation.block_idx === blockIndex
                );
                
                if (!hasRelevantCitation) return null;

                return (
                  <div
                    key={`${diff.pairKey}-${pairIdx}`}
                    className="group bg-purple-50 rounded-md p-1.5 text-xs text-purple-900 leading-snug mt-1 hover:bg-purple-100 transition-colors border border-transparent hover:border-purple-200"
                  >
                    {/* Claim */}
                    <div className="mb-1">
                      <span className="font-medium text-purple-800">Claim: </span>
                      <span>{pair.claim}</span>
                    </div>

                    {/* Evidence */}
                    {pair.evidence && (
                      <div className="mb-1">
                        <span className="font-medium text-purple-800">Evidence: </span>
                        <span>
