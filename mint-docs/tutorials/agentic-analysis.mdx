...
Coding agents can flexibly automate multi-step manual workflows and conduct investigations in parallel. For instance, your agent can support:

- **Comparing between checkpoints**. Suppose you want to answer questions like "What caused a regression between checkpoint A and checkpoint B?" or "These two checkpoints show a quantitative tradeoff on eval scores. What behaviors might explain that?" You can prompt the agent to compare pairs of successful and failed runs in order to identify changes between multiple versions of a model.
