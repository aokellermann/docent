...
import MinimalJudgeResultCard from './MinimalJudgeResultCard';
...
interface MinimalPaginatedResultsListProps {
  ...
  /** Optional map of agent_run_id → display label (e.g. model name) for the group header */
  metadataByAgentRunId?: Record<string, string>;
}

const DEFAULT_PAGE_SIZE = 25;

const MinimalPaginatedResultsList = ({
  agentRunResults,
  activeResultId,
  activeAgentRunId,
  schema,
  ...
  metadataByAgentRunId,
}: MinimalPaginatedResultsListProps) => {
        ...
        {paginatedResults.map((group, index) => {
          ...
          return (
            ...
            >
              ...
              >
                <span>
                  {metadataByAgentRunId?.[group.agent_run_id]
                    ? `${metadataByAgentRunId[group.agent_run_id]} · ${group.agent_run_id.slice(0, 8)}`
                    : `Agent Run ${group.agent_run_id.slice(0, 8)}`}
                </span>
              </div>
              <div
                className={cn(
...
export default MinimalPaginatedResultsList;
