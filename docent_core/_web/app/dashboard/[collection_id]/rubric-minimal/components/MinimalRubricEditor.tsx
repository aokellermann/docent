...
import RunRubricButton from '../../components/RunRubricButton';
import RubricRunDialog from '../../components/RubricRunDialog';
...
interface MinimalRubricEditorProps {
  collectionId: string;
  rubricId: string;
...
export default function MinimalRubricEditor({
  collectionId,
  rubricId,
  rubricVersion,
  onSave,
  ...
  }, [isMinimalMode, searchParams, router, pathname]);

  const { data: availableJudgeModels } = useGetJudgeModelsQuery();
  const availableModelsForPicker = useMemo(() => {
    if (!isMinimalMode) return availableJudgeModels;
    if (!availableJudgeModels) return undefined;
    return availableJudgeModels.filter(
      (model) =>
        model.provider === 'openai' &&
        model.reasoning_effort === 'medium' &&
        (model.model_name === 'gpt-5' ||
          model.model_name === 'gpt-5-mini' ||
          model.model_name === 'gpt5-mini')
    );
  }, [isMinimalMode, availableJudgeModels]);

  useEffect(() => {
    if (!isMinimalMode || !editable) return;
    if (!availableModelsForPicker || availableModelsForPicker.length === 0)
      return;

    setLocalRubric((prev) => {
      if (!prev) return prev;
      const isCurrentModelAllowed = availableModelsForPicker.some(
        (model) =>
          model.provider === prev.judge_model.provider &&
          model.model_name === prev.judge_model.model_name &&
          model.reasoning_effort === prev.judge_model.reasoning_effort
      );

      if (isCurrentModelAllowed) return prev;
      return { ...prev, judge_model: availableModelsForPicker[0] };
    });
  }, [isMinimalMode, editable, availableModelsForPicker]);
  ...
  const handleStartDirectSearch = async () => {
    try {
      ...
      console.error('Failed to start direct search:', error);
      toast.error('Failed to start direct search');
    }
  };
      ...
      <div className="flex flex-row gap-1">
        ...
        {rubric && (
          <ModelPicker
            ...
            availableModels={availableModelsForPicker}
            onChange={(jm) => {
              if (!editable) return;
