...
@user_router.delete("/collections/delete")
async def bulk_delete_collections(
    ...
    """
    Bulk delete collections. Requires direct ADMIN permission on all collections.
    Returns success message on success, or raises 403 if permission check fails or deletion fails.
    """
    # Check if user has direct ADMIN permission on all collections
    has_permission = await mono_svc.get_direct_permissions(
        user=user,
        resource_type=ResourceType.COLLECTION,
        resource_ids=request.collection_ids,
        permission=Permission.ADMIN,
    )
    if not has_permission:
        raise HTTPException(
            status_code=403,
            detail="User lacks ADMIN permission on one or more collections",
        )

    success = await mono_svc.delete_collections(collection_ids=request.collection_ids)
    if not success:
        raise HTTPException(
            status_code=403,
            detail="Failed to delete collections",
        )
...
@user_router.get("/collections/page", response_model=PaginatedCollectionsResponse)
async def get_collections_page(
    ...
    Returns collections the user has access to, ordered by created_at descending (newest first).
    """
    sqla_collections, total = await mono_svc.get_collections_page(
