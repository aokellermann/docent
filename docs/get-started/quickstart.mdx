...
We don't offer a hosted version of Docent yet, but we plan to soon. Luckily, self-hosting is easy and free. For larger organizations, we provide white-glove hosting services; please [reach out](mailto:kevin@transluce.org) if you're interested.
...
You should now have a `.env` file at the project root. See [here](/concepts/environment-variables) for details on how to fill it in.
...
after which Postgres and Redis will be available at the addresses set in [`.env`](/get-started/envvars). To set up your own databases, visit the official docs for [Postgres](https://www.postgresql.org/download/) and [Redis](https://redis.io/docs/latest/operate/oss_and_stack/install/archive/install-redis/).
